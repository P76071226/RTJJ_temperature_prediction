...
def main():
    ...
    for folder in folders:
        ...
        scores["took_trade"] = took
        scores["pnl"] = pnl
        scores["edge_used"] = edge
        scores["trade_side"] = side   # +1 = buy model > market, -1 = sell

        results.append(scores)
        print(f"[{date_str}] Brier={scores['brier']:.4f}  Log={scores['log_score']:.3f}  "
              f"RPS={scores['rps']:.4f}  DirAcc={scores['direction_correct']}  "
              f"TopHit={scores['top_hit']}  Trade={took}  PnL={pnl:.3f}")
