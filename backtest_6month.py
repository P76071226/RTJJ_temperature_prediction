...
model_data = joblib.load('/root/hermes_research/rjtt_project/models/calibrated_xgb_model.pkl')
...
for day_dir in sorted(archive_dir.iterdir()):
    ...
    results.append({
        ...
        'actual_interval': intervals[actual_interval_idx],
        'fc_mean': fc_mean,
        'cloud': cloud_mean,
        'brier': brier,
        'log_score': log_score,
        ...
        'market_dir_correct': market_dir_correct,
        'top_hit': top_hit,
        'market_top_hit': market_top_hit,
        'error': error,
        'pnl': pnl,
        'model_top': intervals[model_top],
        'market_top': intervals[market_top],
    })
